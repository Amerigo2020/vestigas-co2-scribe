...
Interactive Streamlit dashboard for visualizing CSRD compliance CO₂ emissions,
audit trails, and performance metrics with comprehensive filtering and drill-down.

Run with: streamlit run csrd_dashboard.py
"""
...
# Page configuration
st.set_page_config(
    page_title="CSRD CO₂ Emissions Dashboard",
    page_icon="🌱",
    layout="wide",
...
@st.cache_data
def load_data(filepath="csrd_co2e_report_with_conversions.csv"):
    """Load and cache the CSRD report data"""
    try:
...
def main():
    ...
    if df is None or len(df) == 0:
        st.error("❌ Unable to load data. Please ensure csrd_co2e_report_with_conversions.csv exists in the working directory.")
        return
    ...
    with col2:
        st.metric(
            ...
            f"{kpis['material_co2e']/1e6:.1f}M kg",
            delta=None
        )
    ...
    with col3:
        st.metric(
            ...
            f"{kpis['successful_items']}/{kpis['total_items']} items",
            delta=None
        )
